...
namespace FinalFractalSet.Weapons.FinalFractal_Old
{
    public class FinalFractalProjectile : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (LogSpiralLibraryMod.FinalFractalTailEffect != null)
            {
                ...
                if (bars.Count > 2)
                {
                    ...
                    // 把变换和所需信息丢给shader
                    LogSpiralLibraryMod.FinalFractalTailEffect.Parameters["uTransform"].SetValue(model * Main.GameViewMatrix.TransformationMatrix * projection);
                    LogSpiralLibraryMod.FinalFractalTailEffect.Parameters["uTime"].SetValue(-(float)Main.time * 0.03f);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            SpriteBatch spriteBatch = Main.spriteBatch;
            {
                ...
                if (bars.Count > 2)
                {
                    ...
                    {
                        LogSpiralLibraryMod.ColorfulEffect.Parameters["uTransform"].SetValue(model * Main.GameViewMatrix.TransformationMatrix * projection);
                        LogSpiralLibraryMod.ColorfulEffect.Parameters["uTime"].SetValue(-(float)Main.time * 0.03f);
                        LogSpiralLibraryMod.ColorfulEffect.Parameters["defaultColor"].SetValue(Main.hslToRgb(drawColor, 1f, 0.5f).ToVector4());
                        Main.graphics.GraphicsDevice.Textures[0] = LogSpiralLibraryMod.HeatMap[7].Value;
                        Main.graphics.GraphicsDevice.Textures[1] = LogSpiralLibraryMod.BaseTex[13].Value;
                        Main.graphics.GraphicsDevice.Textures[2] = LogSpiralLibraryMod.AniTex[5].Value;
                        Main.graphics.GraphicsDevice.SamplerStates[0] = SamplerState.PointWrap;
    ...
    public class FinalFractalDimensionalSwoosh2 : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.localAI[1] > 7f)
            {
                int dustID = Dust.NewDust(new Vector2(projectile.position.X - projectile.velocity.X * 4f + 2f,
                    ...
                    projectile.oldVelocity.Y, 100, default, 1.25f);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int num798 = 4; num798 < 31; num798 = num3 + 1)
            {
                ...
                int num801 = Dust.NewDust(new Vector2(projectile.oldPosition.X - num799, projectile.oldPosition.Y - num800), 8, 8, MyDustId.RedBubble, projectile.oldVelocity.X, projectile.oldVelocity.Y, 100, default, 1.8f);
                Main.dust[num801].noGravity = true;
                Dust dust = Main.dust[num801];
                dust.noGravity = true;
                dust.velocity *= 0.5f;
                num801 = Dust.NewDust(new Vector2(projectile.oldPosition.X - num799, projectile.oldPosition.Y - num800), 8, 8, MyDustId.RedBubble, projectile.oldVelocity.X, projectile.oldVelocity.Y, 100, default, 1.4f);
