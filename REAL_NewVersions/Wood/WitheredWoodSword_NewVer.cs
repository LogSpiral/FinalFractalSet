...
public class ThornTree_Proj : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (thornTree == null && Projectile.owner == Main.myPlayer) 
        {
            ...
            BuildTree(_useAltSize, _randSeed);
        }
        //Projectile.ai[1]++;
        float k = (1 - MathF.Cos(MathHelper.TwoPi * MathF.Sqrt(Factor))) * .5f;
        if (thornTree == null) return;
        if (thornTreeTiny == null) return;
    ...
    private bool _useAltSize;
    private int _randSeed;
    private void BuildTree(bool altSize, int randSeed)
    {
        UnifiedRandom rand = new(randSeed);
        float stdSize = 64;
        ...
        thornTreeTiny.BuildTree(rand, 8);
    }
    public override void OnSpawn(IEntitySource source)
    {
    ...
    public override void ReceiveExtraAI(BinaryReader reader)
    {
        ...
        {
            _pendingSyncTree = true;
            _useAltSize = reader.ReadBoolean();
...
public class ThornTree(float length, float width, float rotation, ThornTree.TreeGenerateInfo info)
{
    ...
    private class Node(float length, float width, float rotation, bool main)
    {
        ...
        /// 节点末端的宽度
        /// </summary>
        private readonly float width = width;
        ...
        /// 是否处于主干
        /// </summary>
        private readonly bool mainBranch = main;

        private static float GetNormalRandom(UnifiedRandom random, float factor) => (float)random.GaussianRandom(factor, factor * .25 * .33);
        ...
        public void AddToVertex(IList<CustomVertexInfo> vertexInfos, IList<Vector4> EndNodes, float parentWidth, float parentRotation, Vector2 nodePoint, Color color, float factor = 1f)
        {
            ...
            Vector2 normalUnit = new(-unit.Y, unit.X);
    ...
    private readonly Node mainNode = new(length, width, rotation, false);
    private TreeGenerateInfo genInfo = info;
    private int depth;
